...
// spell-checker:ignore (ToDO) delim sourcefiles undelimited foxjumping xacfoxjumping
...
fn parse_range_list(list: &str, is_field: bool) -> Result<Vec<Range>, String> {
    ...
    for item in list.split([',', ' ']) {
        // A stray separator leaves an empty item, which GNU reads as zero.
        let range = if item.is_empty() {
            Range { low: 0, high: 0 }
        } else {
            parse_range(item, errors)?
        };
        if range.low == 0 {
